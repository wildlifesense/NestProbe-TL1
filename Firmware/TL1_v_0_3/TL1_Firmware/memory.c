/********************************************************************************
 ...
 memory.c
	...
	This module was developed for the NestProbe TL1 temperature logger.
...
/*
 * Scan memory array backwards and find the byte address of the first blank word (0xFF).
 * Returns the byte address of the first blank word, which is 0 if memory array is all blank.
 *
 */
	...
	do {
		...
		do {
			if (page_read_buffer[current_byte] != 0xFF) {		// A non-0xFF byte was found ...
				...
				last_data_byte_address &= ~1;	// Cut out last bit to ensure its even address (base of word).
				last_data_byte_address += 2;			// Actually return first non-data byte
